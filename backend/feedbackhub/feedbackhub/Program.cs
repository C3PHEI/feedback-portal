...
app.MapGet("/test-email", async (GraphEmailService emailService) =>{     try    {         await emailService.SendEmail("your.email@company.com");         return Results.Ok("Email sent");     }     catch (Exception ex)     {         return Results.Problem(ex.ToString());     } });
